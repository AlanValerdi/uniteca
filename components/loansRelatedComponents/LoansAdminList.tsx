...
import { approveLoan, rejectLoan, renewLoan, returnBook } from "@/app/lib/loanAdminActions";
import { toast } from "sonner";
  ...
  if (error) {
    ...
    return(
        ...
        >
            ...
            <h1 className="text-3xl font-bold mb-6">Loan Administration</h1>
...
function LoadCard({loan, refetchLoans }: {loan:Loan, refetchLoans: () => void }) {
    ...
    const handleApprove = async () => {
        try {
            ...
            toast.success("You approved the loan successfully");
            refetchLoans();
        }catch(error){
            console.error(error);
        }
    ...
    const handleReject = async () => {
        try {
            ...
            toast.success("You reject the loan succesfully");
            refetchLoans();
        }catch(error){
            console.error(error);
        }
    ...
    const handleRenew = async () => {
        try{
            ...
            toast.success("You renewed the loan successfully");
            refetchLoans();
        }catch(error){
            console.error(error);
        }
        ...
        <Card>
            ...
            <div className="flex flex-col md:flex-row gap-4">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 text-center md:text-left items-center">
                    ...
                    {/* TODO: create the renew and approve end point */}
                    <div className="flex gap-6">
                        { loan.status == "approved" ? (
                            <>
                                <Button variant={'default'} className="bg-green-700 hover:bg-green-800" onClick={handleRenew}>Renew</Button>
                                <Button variant={'default'} className="bg-green-500 hover:bg-green-600" onClick={handleReturn}><span className="font-bold">Mark as returned</span></Button>
                            </>
                        ) : loan.status == "rejected" || loan.status == "returned" ?(
                            <></>
                        ):(
                            <>
                                <Button variant={'destructive'} className="hover:bg-red-800" onClick={handleReject}>Reject</Button>
                                <Button variant={'default'} className="bg-green-600 hover:bg-green-800" onClick={handleApprove}>Approve</Button>
                            </>
                        ) }
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm">
                    <div className="flex space-x-1">
                     <span className="text-muted-foreground">Requested on:</span> <p>{loan.requestDate}</p>
                    </div>
                    <div className="flex space-x-1">
                     <span className="text-muted-foreground">Due date:</span> <p>{loan.dueDate}</p>
                    </div>
                    { !loan.borrowDate  ? (
                        <></>
                    ) : loan.status == "returned" ? (
                        <div className="flex space-x-1">
                            <span className="text-muted-foreground">Returned on:</span> <p>{loan.borrowDate}</p>
                        </div>
                    ) : (
                        <div className="flex space-x-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm">
                  ...
                  <div className="flex space-x-1">
                      <span className="text-muted-foreground">Language:</span> <p>{loan.book.language}</p>
                  </div>
                ...
                <Separator className="my-4" />
                <p className="text-muted-foreground my-4">Requested by:</p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm">
                  ...
                  <div className="flex space-x-1">
                      <span className="text-muted-foreground">Email:</span> <p>{loan.borrower.email}</p>
                  </div>
                </div>
                </div>
            </div>
